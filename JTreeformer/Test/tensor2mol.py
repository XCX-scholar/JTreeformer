...
def dfs_assemble(all_nodes, cur_mol, global_amap, fa_amap, cur_node, fa_node,check_aroma):
    ...
    cands, aroma_score = enum_assemble(cur_node, neighbors, prev_nodes, cur_amap,cs=True,bl=True)
    ...
    if len(cands) > 1:
        scores = torch.Tensor(aroma_score).cuda()
    else:
    ...
    for i in range(cand_idx.numel()):
        ...
        for nei_node in children:
            ...
            tmp_mol, tmp_mol2 = dfs_assemble(all_nodes, cur_mol, new_global_amap,pred_amap, nei_node, cur_node,check_aroma)
            if tmp_mol is None:
                has_error = True
...
def Tensor2MolTree(model:JTreeformer,
        ...
        if prob_decode_node:
            while (True):
                sort_wid = torch.multinomial(prob_node[:,:vocab.vmap['stop']], 5)
        ...
        for i in range(5):
            root_wid = sort_wid[:, i].item()
            if root_wid!=0:
                next_node=sort_wid[:, i]
                break
        ...
        node_list=torch.cat([node_list,next_node.unsqueeze(0)],dim=-1)
...
def MolTree2Mol(pred_root:MolTreeNode, pred_nodes:list):
    ...
    cur_mol, _ = dfs_assemble(pred_nodes, cur_mol, global_amap, [], pred_root, None,check_aroma=True)
    if cur_mol is None:
        cur_mol = copy_edit_mol(pred_root.mol)
        global_amap = [{}] + [{} for node in pred_nodes]
        global_amap[1] = {atom.GetIdx(): atom.GetIdx() for atom in cur_mol.GetAtoms()}
        cur_mol, pre_mol = dfs_assemble(pred_nodes, cur_mol, global_amap, [], pred_root,None,check_aroma=False)
