...
def atom_equal(a1, a2):
    return a1.GetSymbol() == a2.GetSymbol()
...
def enum_attach(ctr_mol, nei_node, amap, singletons,bl=False,be=True):
    ...
    if nei_mol.GetNumBonds() == 0:  # neighbor singleton
        ...
        #         for b in ctr_bonds:
        #             if ring_bond_equal(b, bond):
        #                 new_amap = amap + [(nei_idx, b.GetBeginAtom().GetIdx(), bond.GetBeginAtom().GetIdx()),
        #                                    (nei_idx, b.GetEndAtom().GetIdx(), bond.GetEndAtom().GetIdx())]
        #                 att_confs.append(new_amap)
        #
        #             if ring_bond_equal(b, bond, reverse=True):
        #                 new_amap = amap + [(nei_idx, b.GetBeginAtom().GetIdx(), bond.GetEndAtom().GetIdx()),
        #                                    (nei_idx, b.GetEndAtom().GetIdx(), bond.GetBeginAtom().GetIdx())]
        #                 att_confs.append(new_amap)
    else:
...
def enum_assemble(node,neighbors,prev_nodes=[],prev_amap=[],bl=False,cs=False):
    all_attach_confs = []
    ...
    def search(cur_amap,depth):
        ...
        cand_amap = enum_attach(node.mol, nei_node, cur_amap, singletons,bl,cs)
        cand_smiles = set()
        candidates=[]
